...
const defaultForm = { title: '', description: '', deadline: '', priority: 'medium', category: 'general', time_required: 60 } as any;
...
export function TasksPage() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      // Backend returns full list; client-side paginate for now
  ...
  const onCreate = async () => {
    try {
      const res = await api.post('/api/v1/tasks', { ...form, deadline: form.deadline || null });
      setOpen(false); setForm(defaultForm); await fetchTasks();
    } catch (e: any) {
  ...
  const onUpdate = async () => {
    ...
    try {
      ...
      setOpen(false); setEditingId(null); setForm(defaultForm); await fetchTasks();
  ...
  const onToggleStatus = async (id: string, status: 'pending'|'completed') => {
  ...
  return (
    <Stack gap={2}>
      <Card>
        <CardContent>
          <Stack direction={{ xs: 'column', sm: 'row' }} gap={2} alignItems={{ sm: 'center' }}>
            ...
            <Button startIcon={<AddIcon />} variant="contained" onClick={() => { setForm(defaultForm); setEditingId(null); setOpen(true); }}>New Task</Button>
          </Stack>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent>
          ...
          <Stack direction="row" justifyContent="flex-end" mt={2}>
            {/* Simple pagination controls; since we slice client-side, just next/prev */}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack gap={2} mt={1}>
            <TextField label="Title" value={form.title} onChange={(e) => setForm((f:any) => ({ ...f, title: e.target.value }))} />
            <TextField label="Description" value={form.description} onChange={(e) => setForm((f:any) => ({ ...f, description: e.target.value }))} />
            <TextField type="datetime-local" label="Deadline" value={form.deadline} onChange={(e) => setForm((f:any) => ({ ...f, deadline: e.target.value }))} InputLabelProps={{ shrink: true }} />
            <TextField select label="Priority" value={form.priority} onChange={(e) => setForm((f:any) => ({ ...f, priority: e.target.value }))}>
              <MenuItem value="low">Low</MenuItem>
              <MenuItem value="medium">Medium</MenuItem>
              <MenuItem value="high">High</MenuItem>
              <MenuItem value="urgent">Urgent</MenuItem>
            </TextField>
            <TextField label="Category" value={form.category} onChange={(e) => setForm((f:any) => ({ ...f, category: e.target.value }))} />
            <TextField type="number" label="Time required (min)" value={form.time_required} onChange={(e) => setForm((f:any) => ({ ...f, time_required: Number(e.target.value) }))} />
          </Stack>
        </DialogContent>
        <DialogActions>
