...
export function RecurringTasksPage() {
  ...
  const [form, setForm] = useState<any>({ title: '', description: '', deadline: '', rrule_string: '', priority: 'medium', category: 'general', time_required: 60, end_date: '' });
  const [selectedId, setSelectedId] = useState<string>('');
  const [updateType, setUpdateType] = useState<'this'|'following'|'all'>('all');
  const [deleteType, setDeleteType] = useState<'this'|'following'|'all'>('all');
      ...
      <Card>
        <CardContent>
          ...
          <Stack gap={2} mt={1}>
            ...
            <TextField type="datetime-local" label="First Deadline" value={form.deadline} onChange={(e) => setForm((f:any) => ({ ...f, deadline: e.target.value }))} InputLabelProps={{ shrink: true }} />
            <TextField label="RRULE" placeholder="FREQ=WEEKLY;BYDAY=MO,WE,FR" value={form.rrule_string} onChange={(e) => setForm((f:any) => ({ ...f, rrule_string: e.target.value }))} />
            <TextField label="End date" type="date" value={form.end_date} onChange={(e) => setForm((f:any) => ({ ...f, end_date: e.target.value }))} InputLabelProps={{ shrink: true }} />
        ...
        <CardContent>
          ...
          <Stack gap={2} mt={1}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} gap={2}>
              <TextField select label="Update Type" value={updateType} onChange={(e) => setUpdateType(e.target.value as any)} sx={{ minWidth: 180 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} gap={2}>
              <TextField select label="Delete Type" value={deleteType} onChange={(e) => setDeleteType(e.target.value as any)} sx={{ minWidth: 180 }}>
                <option value="this">this</option>
                <option value="following">following</option>
                <option value="all">all</option>
              </TextField>
